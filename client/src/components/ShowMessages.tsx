...
function ShowMessages({ messages, username, room, users }: Props) {
  ...
  return (
    <div
      className=" px-4 py-2  rounded-md overflow-auto flex-1  flex flex-col shadow-lg w-full h-full "
      style={backgroundImag}
    >
      <div className="font-bold text-2xl shadow-md text-white bg-red-600 w-full  rounded flex justify-between items-center px-3 py-1">
        ...
        <div className=" flex gap-2 items-center">
          <div className="bg-white text-black rounded-md gap-2 flex items-center  px-2 ">
            <BiSolidUserCircle />
            <p>{username}</p>
          </div>
          <div
            ...
            <div
              className={`${
                message.username == username
                  ? "self-end text-white pr-2"
                  : "self-start text-yellow-300 pl-2 "
              } text-xs  w-fit`}
            >
              {message.username}
            </div>
